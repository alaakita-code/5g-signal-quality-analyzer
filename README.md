...
**`plain_scatter` 地圖在手機直向畫面看起來是空白的**
`_plain_scatter()` 原本會依緯度校正 X/Y 軸的縱橫比例(讓經緯度看起來
比例接近真實地理距離),但在手機直向的窄容器下,Plotly 為了同時滿足
「鎖定比例」與「塞進窄容器」兩個條件,會把可視範圍縮到只剩極小一塊,
資料點全部被擠出畫面外,看起來像整個地圖是空的。已移除這個縱橫比例
鎖定,兩軸各自正常縮放,代價是不同螢幕比例下經緯度的視覺比例會有些
微差異,但保證任何裝置都看得到完整資料。
